...
class MyFrame extends JFrame implements ActionListener {
	...
	public MyFrame(String s) {
		...
		frame.add(new JLabel(("Welcome, Player Application is now loading."),
				SwingConstants.CENTER));
		frame.setVisible(true);
		frame.setSize(300, 100);
		window.dispose();

		String url = "rmi://192.168.23.138/";
		try {
			list = (LinkedListExcep) Naming.lookup(url + "Players");
		} catch (Exception e) {
			System.out.println("RMI Error99");
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			System.out.print("RMI Error1");
		}

	}

	public void init_Player_list() throws RemoteException {
		list.insert("Rob Elliot", "GK", 5, 1987);
		list.insert("Seamus Coleman", "RB", 10, 1988);
		list.insert("Ciaran Clark", "CB", 9, 1989);
		list.insert("Shane Duffy", "CB", 1, 1992);
		list.insert("Robbie Brady", "LB", 1, 1992);
		list.insert("Jon Walters", "RM", 9, 1983);
		list.insert("Jeff Hendrick", "CM", 1, 1992);
		list.insert("Harry Arter", "CM", 12, 1989);
		list.insert("Wes Hoolahan", "CM", 5, 1982);
		list.insert("James McClean", "LM", 4, 1989);
		list.insert("Shane Long", "ST", 1, 1987);
		index = 1;
